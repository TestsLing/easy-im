...
use EasyIM\TencentIM\Auth\AccessToken;
...
class BaseClientTest extends TestCase
{
    ...
    public function testHttpUpload()
    {
        ...
        $client->expects()->request($url, 'POST', \Mockery::on(function ($params) use ($query, $path) {
            ...
            self::assertInternalType('resource', $params['multipart'][0]['contents']);
...
//    }
    ...
    public function testAccessTokenMiddleware()
    {
        ...
        $request = new Request('GET', 'http://easyim.cn');
        $options = ['foo' => 'bar'];
...
//        $p = $c->sendAsync(new Request('GET', 'http://easyim.cn'), []);
//        $response = $p->wait();
//
